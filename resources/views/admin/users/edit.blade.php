    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jci_chapter') ? 'is-invalid' : '' }}" name="jci_chapter_id" id="jci_chapter_id">
                    @foreach($jci_chapters as $id => $entry)
                        <option value="{{ $id }}" {{ (old('jci_chapter_id') ? old('jci_chapter_id') : $user->jci_chapter->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('jci_chapter'))
                    <span class="text-danger">{{ $errors->first('jci_chapter') }}</span>
                @endif
