        ...
        <div class="col-md-12">
            @can('listing_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.listings.create') }}">
                            ...
                            <tbody>
                                @foreach($listings as $key => $listing)
                                    <tr data-entry-id="{{ $listing->id }}">
                                        ...
                                        <td>
                                            @can('listing_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.listings.show', $listing->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('listing_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.listings.edit', $listing->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('listing_delete')
                                                <form action="{{ route('frontend.listings.destroy', $listing->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('listing_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.listings.massDestroy') }}",
